#!/usr/bin/env bash
#
# Build the sample-app Docker image, push to ECR, and deploy to EKS.
#
# Usage:
...
sed "s|image: sample-app:latest|image: ${ECR_REPO}:${IMAGE_TAG}|" k8s/deployment.yaml | kubectl apply -f -
...
kubectl rollout status deployment/sample-app -n observability --timeout=120s

echo ""
echo "==> Done! Sample app deployed."
echo ""
echo "Verify:"
echo "  kubectl get pods -n observability -l app=sample-app"
echo "  kubectl logs -n observability -l app=sample-app --tail=20"
echo ""
echo "Port-forward to test locally:"
echo "  kubectl port-forward svc/sample-app 8080:80 -n observability"
